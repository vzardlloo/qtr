...
import type {EngineMeta, EngineName, TranslatorEngine} from './types.js';
...
export async function createEngine(name: EngineName): Promise<TranslatorEngine> {
	...
	switch (name) {
		case 'baidu': {
			...
			if (!creds?.appId || !creds?.appSecret) {
				throw new Error(
					`Baidu engine not configured. Please fill engines.baidu.appId/appSecret in ${configHint}`,
				);
			}
		...
		case 'youdao': {
			...
			if (!creds?.appKey || !creds?.appSecret) {
				throw new Error(
					`Youdao engine not configured. Please fill engines.youdao.appKey/appSecret in ${configHint}`,
				);
			}
		...
		case 'tencent': {
			...
			if (!creds?.secretId || !creds?.secretKey) {
				throw new Error(
					`Tencent engine not configured. Please fill engines.tencent.secretId/secretKey in ${configHint}`,
				);
			}
